...
import java_bg from './images/java/Java_logo_icon.png';


// Define the quiz questions as an array of objects
import { data } from './qansdata/javaqans';

const questions = shuffle(data.questions).slice(0, 4);


// Define the JavaProgrammingQuiz component
export default function JavaProgrammingQuiz({ resetSubject }) {
    ...
    // This component renders the Java Programming Quiz. It displays the current question and answer options if the quiz is not completed, 
    // and the user's final score and options to restart the quiz or choose a different subject if the quiz is completed. 
    return (
        <View style={styles.container}>
            <Text style={styles.title}>Java Programming Quiz</Text>
            {quizCompleted ? (
                <View>
                    <View style={styles.imageContainer}>
                        <Image source={java_bg} resizeMode='center' style={styles.image} />
                    </View>
